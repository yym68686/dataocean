...
const DEFAULT_BASE_URL = "https://vip1.zhunfu.cn";
...
export function getYizhifuConfig() {
  const pid = process.env.YIZHIFU_PID?.trim();
  const merchantPrivateKey = normalizePem(process.env.YIZHIFU_MERCHANT_PRIVATE_KEY);
  const platformPublicKey = normalizePem(process.env.YIZHIFU_PLATFORM_PUBLIC_KEY);
  const baseUrl = (process.env.YIZHIFU_BASE_URL || DEFAULT_BASE_URL).replace(/\/+$/, "");
...
export async function getYizhifuStatus() {
  const config = getYizhifuConfig();
  const [snapshotResult, orderResult] = await Promise.all([
    pool.query("select captured_at, raw from yizhifu_merchant_snapshots order by captured_at desc limit 1"),
    pool.query("select count(*)::int as count, max(updated_at) as last_order_at from yizhifu_orders"),
  ]);
  ...
  return {
    ...
    scheduler: getYizhifuSchedulerStatus(),
  };
}

export function startYizhifuScheduler() {
  if (schedulerState.timer || schedulerState.initialTimer) {
    return getYizhifuSchedulerStatus();
  }

  const enabled = readBooleanEnv("YIZHIFU_SYNC_ENABLED", false);
  const intervalMs = readIntegerEnv("YIZHIFU_SYNC_INTERVAL_MS", DEFAULT_SYNC_INTERVAL_MS);

  schedulerState.enabled = enabled && intervalMs > 0;
  schedulerState.intervalMs = intervalMs;
  schedulerState.maxPages = readIntegerEnv("YIZHIFU_SYNC_MAX_PAGES", DEFAULT_SYNC_MAX_PAGES);
  schedulerState.limit = readIntegerEnv("YIZHIFU_SYNC_LIMIT", DEFAULT_SYNC_LIMIT);
  ...
  const run = () => {
    runScheduledYizhifuSync().catch((error) => {
      schedulerState.lastErrorAt = new Date().toISOString();
      schedulerState.lastError = error.message ?? "Scheduled Yizhifu sync failed";
      console.warn("[yizhifu] scheduled sync failed", schedulerState.lastError);
    });
  };

  const initialDelayMs = readIntegerEnv("YIZHIFU_SYNC_INITIAL_DELAY_MS", 5_000);
  ...
  console.log(
    `[yizhifu] scheduled sync enabled interval=${schedulerState.intervalMs}ms maxPages=${schedulerState.maxPages} limit=${schedulerState.limit}`,
  );

  return getYizhifuSchedulerStatus();
}

export function getYizhifuSchedulerStatus() {
  return {
...
export async function syncYizhifu({ maxPages = 4, limit = 50 } = {}) {
  assertConfigured();

  const merchantInfo = await callYizhifu("/api/merchant/info", {});
  ...
  for (let page = 0; page < pageCount; page += 1) {
    const payload = await callYizhifu("/api/merchant/orders", {
      offset,
      limit: pageLimit,
      status: 1,
    });
    ...
    for (const order of orders) {
      await upsertYizhifuOrder(order, "sync");
    }
...
async function runScheduledYizhifuSync() {
  ...
  try {
    ...
    if (!config.configured) {
      schedulerState.lastErrorAt = new Date().toISOString();
      schedulerState.lastError = "Yizhifu credentials are not configured";
      return;
    }

    const result = await syncYizhifu({
      maxPages: schedulerState.maxPages,
      limit: schedulerState.limit,
    });
...
export async function handleYizhifuNotify(query) {
  const config = assertConfigured();
  const payload = normalizePayload(query);

  if (!verifyPayload(payload, config.platformPublicKey)) {
    const error = new Error("Invalid Yizhifu notify signature");
    error.status = 400;
    throw error;
  }

  if (payload.trade_status !== "TRADE_SUCCESS") {
    const error = new Error("Ignored non-success Yizhifu notification");
    error.status = 202;
    throw error;
  }

  await upsertYizhifuOrder({ ...payload, status: 1 }, "notify");
  return { ok: true };
}

export async function queryYizhifuMetric({ dataSource, metric, query }) {
  const key = metric.key;

  if (key === "yizhifu_recent_orders") {
    return createRecentOrdersResult({ dataSource, metric });
  }

  if (key === "yizhifu_revenue_trend") {
...
export async function listYizhifuOrders({ limit = 50 } = {}) {
  const result = await pool.query(
    `
...
export async function getYizhifuSummary() {
  const snapshot = await getLatestSnapshot();
  ...
  const result = await pool.query(
    `
      with snapshot_candidates as (
        ...
        where ($1::timestamptz is null or captured_at::date >= $1::timestamptz::date)
        group by captured_at::date

        union all

        select
          (captured_at::date - 1) as day,
          max(coalesce(order_money_lastday, 0)) as revenue,
          max(coalesce(order_num_lastday, 0)) as orders
        from yizhifu_merchant_snapshots
        where ($1::timestamptz is null or (captured_at::date - 1) >= $1::timestamptz::date)
        group by (captured_at::date - 1)
      ),
      daily_snapshots as (
        select
          day,
      ...
      daily_paid_orders as (
        select
          coalesce(endtime, addtime, updated_at)::date as day,
          coalesce(sum(money), 0) as paid_revenue,
          count(*)::int as paid_orders
        from yizhifu_orders
        where status = 1
          and ($1::timestamptz is null or coalesce(endtime, addtime, updated_at)::date >= $1::timestamptz::date)
        group by coalesce(endtime, addtime, updated_at)::date
      )
      select
...
async function callYizhifu(path, params) {
  ...
  try {
    ...
    const error = new Error(`Invalid Yizhifu JSON response: ${text.slice(0, 160)}`);
    error.status = 502;
    throw error;
  }

  if (!response.ok || Number(json.code) !== 0) {
    const error = new Error(json.msg || `Yizhifu request failed with ${response.status}`);
    error.status = 502;
    error.payload = json;
    throw error;
  }
  ...
  if (json.sign && !verifyPayload(json, config.platformPublicKey)) {
    const error = new Error("Invalid Yizhifu response signature");
    error.status = 502;
    error.payload = json;
    throw error;
  }
...
async function insertMerchantSnapshot(payload) {
  await pool.query(
    `
      insert into yizhifu_merchant_snapshots (
...
async function upsertYizhifuOrder(payload, source) {
  ...
  await pool.query(
    ...
    [
      ...
      parseYizhifuTime(payload.addtime),
      parseYizhifuTime(payload.endtime),
      source,
      JSON.stringify(payload),
    ],
  );
...
function assertConfigured() {
  const config = getYizhifuConfig();
  if (!config.configured) {
    const error = new Error("Yizhifu is not configured. Set YIZHIFU_PID, YIZHIFU_MERCHANT_PRIVATE_KEY, and YIZHIFU_PLATFORM_PUBLIC_KEY.");
    error.status = 409;
    throw error;
  }
  return config;
...
function parseYizhifuTime(value) {
  if (!value) {
    return null;
  }
